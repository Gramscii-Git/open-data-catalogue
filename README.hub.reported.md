...
The retained discovery archive **$catalogue_status** the policy recorded in its
[original published report]($catalogue_quality_url). The table below reproduces
those historical measurements. The report is preserved byte for byte in
[catalogue-quality.json](catalogue-quality.json), revision
`$catalogue_report_revision`, SHA-256 `$catalogue_report_sha256`.

Current admission is **not granted**. The archive declares snapshot schema
**$catalogue_observed_schema**; the current release contract requires schema
**$catalogue_required_schema** and a successful current quality evaluation.
This documentation update performs **no current quality evaluation** and does
not provide missing document provenance or certify the old export. The separate
[current status]($catalogue_status_url) binds both immutable publications and the
current policy SHA-256 `$catalogue_policy_sha256`. The historical acceptance
flag is not admission under the current policy.
