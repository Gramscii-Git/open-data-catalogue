...
def load(path):
    ...
    if set(config) != {"schema_version", "tables"} or config["schema_version"] != 1:
    ...
    for table in tables:
        ...
        if table["archive"] not in {"catalogue", "availability"} or type(table["default"]) is not bool:
    ...
    for table in tables:
        ...
        with tarfile.open(archives[table["archive"]], "r:gz") as archive, output.open("x", encoding="utf-8") as target:
            for line in archive.extractfile(table["member"]):
                ...
                count += 1
        member = table["member"] if table["archive"] == "availability" else table["member"].removesuffix(".jsonl")
        ...
        files.append({"path": table["path"], "rows": count, "sha256": digest,
                      "bytes": output.stat().st_size, "source_sha256": report["sha256"]})
